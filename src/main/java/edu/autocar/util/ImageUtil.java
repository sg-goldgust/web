...
public class ImageUtil {
	...
	public static byte[] thumb(byte[] data, int width, int height) throws Exception {
		try (ByteArrayInputStream in = new ByteArrayInputStream(data);
			...
			Thumbnails.of(in).size(width, height).crop(Positions.CENTER).toOutputStream(out);
	...
	public static void thumb(File srcFile, File thumbFile, int width, int height) throws Exception {
		Thumbnails.of(srcFile).size(width, height).crop(Positions.CENTER).outputFormat("jpg").toFile(thumbFile);
	}
}
